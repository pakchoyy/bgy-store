...
export async function sendPaymentReminderEmail({ to, buyerName, productTitle, amount, paymentUrl }) {
  if (!hasEmailSender() || !to || !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(to) || !/^https:\/\//.test(paymentUrl || '')) return { skipped: true }
  const from = process.env.EMAIL_FROM || 'Bantu Guru Yuk <onboarding@resend.dev>'
  const rupiah = `Rp${Number(amount || 0).toLocaleString('id-ID')}`
  const html = `<!doctype html><html><body style="margin:0;background:#f0fdfa;font-family:Arial,Helvetica,sans-serif;color:#0f172a">
<table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="padding:24px 12px"><tr><td align="center">
<table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="max-width:480px;background:#ffffff;border-radius:16px;padding:28px"><tr><td>
<p style="margin:0 0 4px;font-size:12px;font-weight:bold;letter-spacing:1px;color:#0d7a8a;text-transform:uppercase">Bantu Guru Yuk</p>
<h1 style="margin:0 0 12px;font-size:21px;line-height:1.3">Pesananmu masih menunggu pembayaran</h1>
<p style="margin:0 0 16px;font-size:15px;line-height:1.6;color:#475569">Halo ${escapeHtml(buyerName || 'Bapak/Ibu')}, pesanan <strong>${escapeHtml(productTitle || 'produk')}</strong> (${escapeHtml(rupiah)}) belum dibayar. Lanjutkan pembayaran supaya file bisa langsung diunduh.</p>
<a href="${escapeHtml(paymentUrl)}" style="display:block;background:#10b981;color:#ffffff;text-decoration:none;text-align:center;font-weight:bold;font-size:15px;padding:14px 20px;border-radius:12px">Lanjutkan Pembayaran</a>
<p style="margin:16px 0 0;font-size:12px;line-height:1.6;color:#64748b">Sudah bayar atau berubah pikiran? Abaikan email ini. Butuh bantuan? Balas email ini.</p>
  ...
  const response = await fetch('https://api.resend.com/emails', {
    ...
    body: JSON.stringify({
      from,
      to: [to],
      subject: `Lanjutkan pembayaran: ${productTitle || 'pesanan kamu'}`,
      html,
      ...(process.env.EMAIL_REPLY_TO ? { reply_to: process.env.EMAIL_REPLY_TO } : {}),
    }),
  })
