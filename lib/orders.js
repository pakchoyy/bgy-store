...
const FILE_COLUMNS = 'id, title, file_url, file_path, file_name'

async function selectProducts(supabase, ids, withBundle) {
  if (!ids.length) return []
  const columns = withBundle ? `${FILE_COLUMNS}, bundle_product_ids` : FILE_COLUMNS
  const { data, error } = await supabase.from('products').select(columns).in('id', ids)
  if (error && withBundle) return selectProducts(supabase, ids, false)
  return data || []
}

export async function getOrderDownloads(supabase, order) {
  if (!order?.product_id) return []
  const items = await getOrderItems(supabase, order)
  const baseIds = items.length ? items.map((i) => i.product_id).filter(Boolean) : [order.product_id]
  const base = await selectProducts(supabase, baseIds, true)
  const bundleIds = [...new Set(base.flatMap((p) => (Array.isArray(p.bundle_product_ids) ? p.bundle_product_ids : [])))]
    .filter((id) => !baseIds.includes(id))
  const included = await selectProducts(supabase, bundleIds, false)

  const hasFile = (p) => !!(p.file_url || p.file_path)
  const ordered = [
    ...baseIds.map((id) => base.find((p) => p.id === id)).filter(Boolean)
      .filter((p) => hasFile(p) || !(Array.isArray(p.bundle_product_ids) && p.bundle_product_ids.length)),
    ...bundleIds.map((id) => included.find((p) => p.id === id)).filter(Boolean),
  ]
  return ordered.map((p) => ({ product_id: p.id, title: p.title, file_url: p.file_url || null, file_name: p.file_name || null, has_file: hasFile(p) }))
}
