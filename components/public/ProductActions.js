...
export default function ProductActions({ product, settings, waUrl }) {
  ...
  return <>
    <div className={!isFree && !soldOut ? 'grid gap-3 sm:grid-cols-[1fr_auto]' : ''}>
      <button type="button" id="main-buy-button" data-track-click data-product-id={product.id} disabled={soldOut} onClick={openOrder} className={`relative z-10 w-full min-h-12 px-6 py-3 rounded-xl text-sm font-bold text-white shadow-lg transition-transform duration-150 active:scale-[0.96] disabled:bg-gray-400 disabled:shadow-none bg-emerald-700 hover:bg-emerald-800 shadow-emerald-600/25`}>{soldOut ? 'Stok Habis' : isFree ? 'Download Gratis' : product.purchase_button_label || 'Beli Sekarang'}</button>
      {!isFree && !soldOut && (
    ...
    {open && !isFree && <BuyModal product={product} waUrl={waUrl} isOpen onClose={() => setOpen(false)} />}
  </>;
}
