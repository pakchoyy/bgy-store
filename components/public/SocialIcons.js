...
function normalizeUrl(platform, url) {
  if (!url) return '#'
  if (platform === 'email' && !url.startsWith('mailto:')) return `mailto:${url}`
  if (platform === 'whatsapp' && !url.includes('http') && !url.includes('wa.me')) {
  ...
  return (
    <div className={`flex items-center justify-center gap-3 ${className}`}>
      {visible.map((link) => {
        ...
        const isStroke = platform === 'website'
        return (
          <a
            key={link.id || `${platform}-${link.url}`}
            href={normalizeUrl(platform, link.url)}
            target="_blank"
